...
def print_vmd_script_nci(scriptfile, densfile, rdgfile, isosurf=0.5, denscut=0.05):
    ...
    output += _vmd_script_molecule('CPK', densfile, rdgfile)
...
def print_vmd_script_isosurface(scriptfile, isofile, colorfile=None, isosurf=0.5, material='Opaque',
    ...
    if colorfile is not None:
        output += _vmd_script_molecule(representation, colorfile, isofile)
        file_index = 1
    else:
        output += _vmd_script_molecule(representation, isofile)
        file_index = 0
...
def print_vmd_script_topology(fname, points, radius=0.2):
    ...
    output = ('#!/usr/local/bin/vmd\n'
              ...
              '#\n'
              '# Critical Points\n')
    for color, coordinates in list(points.items()):
        output += 'draw color {}\n'.format(color)
