...
def test_global_quadratic_nnp_grand_potential_n():
    # E(N) = -9.0 + (-25.0)*N + N^2, N0=15
    grand = lambda n: (-9.0 - 25.0 * n + n * n) - (-25.0 + 2 * n) * n
    # build global tool
    model = QuadraticGlobalTool(-159.0, -153.0, -163.0, 15)
...
def test_global_quadratic_nnp_grand_potential_mu():
    ...
    np.testing.assert_almost_equal(domega_mu(deriv(16.67), 2), -0.5, decimal=6)
    np.testing.assert_almost_equal(domega_mu(deriv(14.31), 3), 0.0, decimal=6)
    np.testing.assert_almost_equal(domega_mu(deriv(12.67), 4), 0.0, decimal=6)
...
def test_global_quadratic_pnp_grand_potential_n():
    # E(N) = 30.0 + (-6.0)*N + 3*N^2, N0=10
    grand = lambda n: (30.0 - 6.0 * n + 3 * n * n) - (-6.0 + 6 * n) * n
    # build global tool
    model = QuadraticGlobalTool(75.0, 102.0, 54.0, 5)
    # check grand potential (as a function of N)
...
def test_global_quadratic_pnp_grand_potential_mu():
    ...
    np.testing.assert_almost_equal(domega_mu(deriv(4.89), 2), -1/6., decimal=6)
    np.testing.assert_almost_equal(domega_mu(deriv(6.79), 3), 0., decimal=6)
    np.testing.assert_almost_equal(domega_mu(deriv(3.12), 4), 0., decimal=6)
...
def test_global_quadratic_n0p_grand_potential_n():
    # E(N) = -100 + 5*N^2, N0=5
    grand = lambda n: (-100.0 + 5 * n * n) - (10 * n) * n
    # build global tool
    model = QuadraticGlobalTool(25.0, 80.0, -20.0, 5)
    # check grand potential (as a function of N)
    np.testing.assert_almost_equal(model.grand_potential(5.), grand(5.), decimal=6)
...
def test_global_quadratic_n0p_grand_potential_mu():
    ...
    np.testing.assert_almost_equal(domega_mu(deriv(3.678), 2), -0.1, decimal=6)
    np.testing.assert_almost_equal(domega_mu(deriv(4.341), 2), -0.1, decimal=6)
    np.testing.assert_almost_equal(domega_mu(deriv(2.001), 3), 0., decimal=6)
    np.testing.assert_almost_equal(domega_mu(deriv(5.456), 4), 0., decimal=6)
